...
public class Tarefa extends Thread {
    ...
    // contadorExec conta o número de execuções
    ...
    //Emula a execução de uma Tarefa. Recebe a lista de Tarefas "original"- list- , a lista de tarefas
    //com a fila de execução - listaExec - e todo o tempo é medido dentro deste método
    ...
    public long execucaoTarefa(Tarefa t, Instant inicio, List<Tarefa> listaExec, List<Tarefa> list) {
        if (listaExec.contains(t)) {
            ...
            //Este While é responsável por mantêr a duração da tarefa em 30s.  
            ...
            listaExec.remove(t);
            //Atualiza os marcadores de "contexto" de uma tarefa
            for (Tarefa f : list) {
    ...
    //método aplicado em PrioD. 
    public void mudaPrioridade(Tarefa t) {
        t.setPrioridade(t.getPrioridade() + 1);
    }
    // O método verificaStatus é responsável por verificar se uma tarefa atingiu o tempo mínimo de pronto,
    //e se ela já foi executada alguma vez ou não
    ...
    //Este método emula uma execução de tarefa
    public void printTarefa(Tarefa t, long contador) {
        System.out.println("-------------------------------");
        System.out.println("EXECUTANDO A TAREFA:" + t.getNomeId() + "| No tempo:" + contador + " |Tempo PRONTO :" + t.getTempoPronto());
        System.out.println("_______________________________");
    }
    //O método filaExecução é responsável pela avaliação da próxima tarefa a ser executada. 
    //Neste caso ele executará sempre a que está há mais tempo pronta.  
    public Tarefa filaExecucao(List<Tarefa> list, long duracao, List<Tarefa> listaExec) {
        ...
        for (Tarefa t : listaExec) {
            if (((t.getTempoPronto()<=auxiliar.getTempoPronto()) &&(t.getTempoPronto() <= duracao)) && (t.getStatus() == 1) && (t.getFlagExec() == 0)) {
                auxiliar = t;
            }
        }
        for (Tarefa t : list) {
...
//Obtém a lista original de tarefas (mais utilizado no algoritmo de PRIOd. 
